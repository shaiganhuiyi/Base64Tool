...
std::string Base64::Encode(const std::string& strData)
{
	...
	std::string strEncodeTable("ABCDEFGHIJKLMNOPQRSTUVMXYZabcdefghijklmnopqrstuvmxyz0123456789+/");
	...
	for (size_t i = 0; i < strData.size()/3;++i)
	{
		...
		//去第一个字节的高6位
		 ...
		 //取第一个字节的低两位，与第二个字节的高4位拼接成6个比特位
		 strEncode +=strEncodeTable[((temp[1] << 4 | (temp[2] >> 4)) & 0x3F)];

		 //取第二个的低4位，与第三个字节的高4位拼接成6个比特位
...
std::string Base64::Decode(const std::string& strData)
{
	...
	{
		// 注意：base64编码时一行最多放置76个字符，超过76个字符时，放置到下一行
		if (strData[index] != '\r' && strData[index + 1] != '\n')
		{
		  //一行没有解码完毕
		  //解析第一个编码-->以该编码作为解码表的下标，到解码表中找该编码表中的下标
